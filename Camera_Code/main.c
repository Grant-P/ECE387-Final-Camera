...
enum states {initial, waitRise, takePic, savePic, waitFall};
int stateSM, nState;
...
void tickSM(void){
	...
	switch(stateSM){ //Transitions
		case initial:
			nState = waitRise;
			break;
		case waitRise:
			if (buttonHigh()) nState = takePic;
			else stateSM = waitRise;
			break;
		case takePic:
			nState = savePic;
			break;
		case savePic:
			...
			break;
		case waitFall:
			if (buttonHigh()) nState = waitFall;
			else nState = waitRise;
			break;
	}
